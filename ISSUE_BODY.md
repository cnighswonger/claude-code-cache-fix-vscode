...
Tested v0.5.0 (and the proxy-mode flow added in v0.5.0/0.5.1 release notes) on Windows 11 + VS Code Insiders + Volta-managed Node 22.22.0 + claude-code-cache-fix npm 3.0.1. Found three bugs that combine into a fairly broken first-run experience for proxy mode. Patch at the bottom — feel free to take any subset.
